...
def build_generation_prompt(report_date: str, news: list[dict[str, str]], case: dict[str, Any]) -> str:
    base = PROMPT_FILE.read_text(encoding="utf-8").replace(
        "{{REPORT_DATE}}",
        report_date,
    )
...
生成新的晨报时，必须主动检查最近7期内容。

必须遵守：

1. theme（大标题）不得重复。

2. terms（5个投资名词）尽量不要与最近7期重复；如果必须重复，必须从新的角度解释。

3. question（思考题）不得重复。

4. company_case（公司案例）不得连续两期重复。

5. summary 必须体现今天新增的信息，而不是重复昨天的导语。

如果新闻变化较小，可以保持课程结构，但 theme、market_flashes、terms、question 必须优先保证与最近7期不同。
如果无法避免重复，必须提供新的解释角度、不同的案例，或新的应用场景。
...
每天晨报首先是一堂投资课，其次才是一份新闻整理。新闻只是案例，真正要更新的是读者的认知。
